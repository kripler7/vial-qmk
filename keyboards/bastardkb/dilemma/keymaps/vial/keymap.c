...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    //,-----------------------------------------------------.                    ,-----------------------------------------------------.
    KC_LBRC,    KC_Q,    KC_W,    KC_F,    KC_P,    KC_B,                         KC_Y,    KC_U,    KC_I,    KC_O,   KC_P,  KC_BSPC,
    //|--------+--------+--------+--------+--------+--------|                    |--------+--------+--------+--------+--------+--------|
      KC_BSLS,    KC_A,    LSFT(KC_R),    KC_D,    KC_F,    KC_G,                         KC_H,    KC_J,    KC_K,    KC_L, KC_SCLN, KC_QUOT,
    //|--------+--------+--------+--------+--------+--------|                    |--------+--------+--------+--------+--------+--------|
...
const uint16_t PROGMEM encoder_map[][NUM_ENCODERS][NUM_DIRECTIONS] = {
	...
	[_SNIPER] = { ENCODER_CCW_CW(_______, _______), ENCODER_CCW_CW(_______, _______) }  

};
#endif

report_mouse_t pointing_device_task_user(report_mouse_t report) {
    float scale = 0.25f;  // New default precision

    if (layer_state_is(_SNIPER)) {
        scale = 0.125f;  // Slower for sniper layer
    }

    report.x = (int16_t)(report.x * scale);
    report.y = (int16_t)(report.y * scale);
    return report;
}
